...
export default function TrackScreen() {
  ...
  const booking = activeBooking ?? { providerName: 'Ravi Kumar', service: 'Kitchen sink repair', eta: '12 min', price: '₹700–₹1,000' };
  return (
    <Screen>
      ...
      <View style={[styles.mapCard, { backgroundColor: colors.secondary }]}>
        <View style={styles.mapGrid}><View style={styles.routeLine} /><View style={styles.routeLineTwo} /></View>
        <View style={styles.mapPill}><Feather name="navigation" size={15} color={colors.primary} /><Text style={[styles.mapPillText, { color: colors.foreground }]}>Live location</Text></View>
        <View style={[styles.pin, { backgroundColor: colors.primary }]}><Feather name="tool" size={16} color={colors.primaryForeground} /></View>
        <View style={[styles.homePin, { backgroundColor: colors.accent }]}><Feather name="home" size={15} color={colors.accentForeground} /></View>
        <Text style={[styles.mapLabel, { color: colors.mutedForeground }]}>Indiranagar</Text>
      </View>
      <View style={styles.etaRow}><View><Text style={[styles.eyebrow, { color: colors.primary }]}>ON THE WAY</Text><Text style={[styles.etaTitle, { color: colors.foreground }]}>Arriving in {booking.eta}</Text><Text style={[styles.etaCopy, { color: colors.mutedForeground }]}>{booking.providerName} is heading to you</Text></View><Tag tone="green">VERIFIED</Tag></View>
      <View style={[styles.providerCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={{ flex: 1 }}><Text style={[styles.providerName, { color: colors.foreground }]}>{booking.providerName}</Text><Text style={[styles.providerMeta, { color: colors.mutedForeground }]}>Trust score 94 · White Honda Activa</Text></View>
      ...
      <View style={[styles.timeline, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <TimelineItem icon="navigation" title="Provider is on the way" meta={`Estimated arrival in ${booking.eta}`} active />
