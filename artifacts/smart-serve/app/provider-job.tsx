...
export default function ProviderJobScreen() {
  ...
  const accept = () => {
    setActiveBooking({ id: `provider-job-${Date.now()}`, providerName: 'Aditi Sharma', service: 'Kitchen sink leakage', status: 'confirmed', eta: '18 min', price: '₹700–₹1,000', location: 'Indiranagar, Bengaluru' });
    router.push('/');
  };
  return (
    <Screen>
      ...
      <View style={[styles.hero, { backgroundColor: colors.secondary }]}>
        ...
        <Text style={[styles.heroTitle, { color: colors.foreground }]}>Kitchen sink leakage</Text>
        <Text style={[styles.heroMeta, { color: colors.mutedForeground }]}>1.8 km away · Today, 10:30 AM · Normal priority</Text>
      </View>
      <View style={[styles.customerCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={{ flex: 1 }}><Text style={[styles.customerName, { color: colors.foreground }]}>Aditi Sharma</Text><Text style={[styles.customerMeta, { color: colors.mutedForeground }]}>Verified customer · 2 previous services</Text></View>
        <Feather name="shield" size={18} color={colors.primary} />
      </View>
      <Text style={[styles.sectionTitle, { color: colors.foreground }]}>What Smart Serve knows</Text>
      <View style={[styles.detailCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Detail icon="droplet" label="Possible issue" value="Loose connection under kitchen sink" />
        <Detail icon="clock" label="Expected duration" value="30–60 minutes" />
        <Detail icon="credit-card" label="Transparent estimate" value="₹700–₹1,000" />
        <Detail icon="map-pin" label="Customer location" value="Indiranagar, Bengaluru" />
      </View>
      ...
      <PrimaryButton onPress={accept} icon="check">Accept job request</PrimaryButton>
